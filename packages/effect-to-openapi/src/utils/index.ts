...
/**
 * The value of a key when it is a number, `undefined` otherwise.
 *
 * @example
 * numberValue({ minLength: 1 }, 'minLength') // 1
 */
export function numberValue(record: { readonly [key: string]: unknown }, key: string) {
  const value = record[key]
  return typeof value === 'number' ? value : undefined
}

/**
 * The value of a key when it is a string, `undefined` otherwise.
 *
 * @example
 * stringValue({ format: 'uuid' }, 'format') // 'uuid'
 */
export function stringValue(record: { readonly [key: string]: unknown }, key: string) {
  const value = record[key]
