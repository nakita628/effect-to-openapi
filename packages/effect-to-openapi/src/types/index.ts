import type { Schema as EffectSchema, SchemaAST } from 'effect'
...
// The object model lives in `openapi/index.ts` (hono-takibi style: `Schema`, `PathItem`, ...).
...
/**
 * Any Effect schema accepted as input.
 */
export type AnySchema = EffectSchema.Top

/**
 * Maps a sub-AST to its generated schema; failures propagate as `{ ok: false, error }`.
 */
...
/**
 * SchemaObject keywords that can be attached through `.annotate()`. `exclusiveMinimum` /
...
export type SchemaMetadata = {
  ...
  readonly prefixItems?: readonly (SchemaObject | ReferenceObject)[]
  readonly properties?: { readonly [propertyName: string]: SchemaObject | ReferenceObject }
...
export type MediaTypeConfig = {
  readonly schema?: AnySchema | SchemaObject | ReferenceObject
   ...
   */
  readonly itemSchema?: AnySchema | SchemaObject | ReferenceObject
...
export type ResponseConfig = {
  ...
  readonly headers?: AnySchema | HeadersObject
  readonly links?: { readonly [k: string]: Link | Reference }
  readonly content?: ContentConfig
}

export type RouteParameter = AnySchema | undefined
...
export type RouteConfig = Omit<Operation, 'responses'> & {
  ...
  readonly request?: {
    ...
    readonly headers?: RouteParameter | readonly AnySchema[]
  }
  readonly responses: {
...
export type OpenAPIDefinition =
  ...
  | { readonly type: 'schema'; readonly schema: AnySchema }
  | { readonly type: 'parameter'; readonly schema: AnySchema }
  | { readonly type: 'route'; readonly route: RouteConfig }
  | WebhookDefinition

export type Definition = OpenAPIDefinition | AnySchema
